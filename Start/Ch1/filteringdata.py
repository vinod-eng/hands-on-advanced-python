...
def is_summer_rain_day(d):
    summer_months = ["-07-","-08-"]
    if any([m in d['date'] for m in summer_months]) and d['prcp']>=1:
        return True
    
    return False

rainyDayInSummer = list(filter(is_summer_rain_day, weatherdata))

print(len(rainyDayInSummer))
pprint.pp(rainyDayInSummer)
